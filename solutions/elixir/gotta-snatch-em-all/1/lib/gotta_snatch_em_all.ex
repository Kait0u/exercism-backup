defmodule GottaSnatchEmAll do
  ...
  def add_card(card, collection) do
    new_collection = MapSet.put(collection, card)
  ...
  def boring_cards(collections) do
    collections
    |> Enum.reduce(fn acc, collection -> MapSet.intersection(acc, collection) end)
    |> MapSet.to_list()
  end
  ...
  def split_shiny_cards(collection) do
    ...
    {MapSet.to_list(shinies), MapSet.to_list(MapSet.difference(collection, shinies))}
  end
end
